...
const _FEATURES = [
  { icon: (<svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M15 3h6v6M9 21H3v-6M21 3l-7 7M3 21l7-7"/></svg>), title: 'Pixel-Perfect', desc: 'Set exact width and height with aspect-ratio lock.' },
  { icon: (<svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="1.8" strokeLinecap="round"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>), title: 'Quality Control', desc: 'Set JPEG output quality after resizing.' },
  { icon: (<svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="1.8" strokeLinecap="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg>), title: '100% Private', desc: 'Canvas-based, nothing uploaded.' },
  { icon: (<svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="1.8" strokeLinecap="round"><path d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>), title: 'Instant', desc: 'Resize finishes in milliseconds.' },
...
export default function ResizeJpgPage() {
  ...
  const faqs = [
    {
      q: "Does resizing a JPG decrease quality?",
      a: "No, we use bicubic canvas resampling to retain clarity. Tweak the quality settings to optimize the output file size without noticeable compression noise."
    },
    {
      q: "Can I specify custom dimensions?",
      a: "Yes, you can enter custom width and height in pixels, or toggle the percentage slider to quickly scale your JPG down."
    },
    {
      q: "Can I lock the aspect ratio?",
      a: "Yes, checking 'Lock aspect ratio' ensures that changing the width automatically updates the height proportionally, preventing image stretching."
    },
    {
      q: "Are my JPG files secure?",
      a: "Yes, all processing is performed locally in your web browser. Your JPG files are never sent to external servers."
    }
  ];
  ...
  useEffect(() => {
    ...
    img.onerror = () => {
      setErrorMsg('Failed to load JPG preview. File may be unsupported or corrupted.');
    };
  ...
  const executeResize = () => {
    ...
    img.onload = () => {
      try {
        ...
        const targetMime = getMimeForSaveFormat(saveFormat, 'image/jpeg');
        ...
        compressCanvasToBlob(canvas, targetMime, targetBytes)
          .then(({ blob, mime }) => {
            ...
            saveHistory('Resize JPG', `${resizedName} (${targetWidth}x${targetHeight})`);
          })
          .catch((err) => {
            console.error(err);
    ...
    img.onerror = () => {
      setErrorMsg('Failed to process JPG source.');
      setIsResizing(false);
    };
  ...
  return (
    <ToolPageShell
      ...
      steps={_STEPS}
      faqs={_FAQS}
      seoText="Resize JPEG images online for free. Set exact pixel dimensions with aspect-ratio lock. Browser-based, no uploads."
    >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-3" style={{ background: "#fff", border: "1px solid #E4E4EF", borderRadius: 20, padding: "20px", boxShadow: "0 2px 12px rgba(0,0,0,0.04)", height: "fit-content", display: "flex", flexDirection: "column", gap: 14 }}>
              <div className="flex justify-between items-center pb-2 border-b border-bordercolor">
                <h3 style={{ fontSize: 10, fontWeight: 800, color: "#9898B5", textTransform: "uppercase", letterSpacing: "0.08em" }}>Uploaded JPGs</h3>
              ...
              <h4 style={{ fontSize: 10, fontWeight: 800, color: "#9898B5", textTransform: "uppercase", letterSpacing: "0.08em" }}>Dimensions Config</h4>
                  ...
                  {!resizeAsPercentage && (
                    <div className="flex items-center gap-2 pt-1">
                      ...
                      />
                      <label htmlFor="ratioLock" className="text-xs font-bold text-textmain select-none cursor-pointer">Lock aspect ratio</label>
                    </div>
                  )}
