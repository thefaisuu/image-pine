...
const _FEATURES = [
  { icon: (<svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="1.8" strokeLinecap="round"><path d="M6 2v14a2 2 0 002 2h14M18 22V8a2 2 0 00-2-2H2"/></svg>), title: 'Freeform Crop', desc: 'Select any rectangular region.' },
  { icon: (<svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="1.8" strokeLinecap="round"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/></svg>), title: 'Transparency Preserved', desc: 'Alpha channel maintained in output.' },
...
export default function CropPngPage() {
  ...
  const handleFileSelect = (selectedList) => {
    ...
    if (selectedList.length > 0) {
      ...
      if (!nameLower.endsWith('.png') && selected.type !== 'image/png') {
  ...
  const handleMouseMove = (e) => {
    ...
    if (dragMode === 'move') {
      ...
      if (corner === 'bottom-right' || dragMode === 'resize') {
        ...
        nextX = dragStart.boxX + dragStart.boxX - nextX; // wait, let's keep exact formula:
        nextX = dragStart.boxX + deltaX;
        nextY = dragStart.boxY + deltaY;
        nextW = dragStart.boxW - deltaX;
        nextH = dragStart.boxH - deltaY;
      ...
      try {
        ...
        canvas.toBlob(
          (blob) => {
            ...
            // Log tool history
            const croppedName = file.name.replace(/\.[^/.]+$/, '') + `_cropped.png`;
            saveHistory('Crop PNG', `${croppedName} (${Math.round(sourceWidth)}x${Math.round(sourceHeight)})`);
          },
          'image/png'
        );
      } catch (err) {
  ...
  const downloadCroppedImage = () => {
    ...
    const newName = `${baseName}_cropped_${getNaturalCoord(cropBox.width, 'width')}x${getNaturalCoord(cropBox.height, 'height')}.png`;
  ...
  return (
    <ToolPageShell
      ...
      steps={_STEPS}
      faqs={_FAQS}
      seoText="Crop PNG images online for free. Trim PNG to any region while preserving transparency. Browser-based, no uploads."
    >
