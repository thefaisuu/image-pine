...
const _FEATURES = [
  { icon: (<svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M15 3h6v6M9 21H3v-6M21 3l-7 7M3 21l7-7"/></svg>), title: 'WebP Resize', desc: 'Resize WebP while keeping the efficient format.' },
  { icon: (<svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="1.8" strokeLinecap="round"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>), title: 'Quality Control', desc: 'Adjust output quality for lossy WebP.' },
  ...
  { icon: (<svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="1.8" strokeLinecap="round"><path d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>), title: 'Instant', desc: 'Fast canvas resize.' },
...
export default function ResizeWebpPage() {
  ...
  const [saveFormat, setSaveFormat] = useState('WebP');
  ...
  const faqs = [
    {
      q: "What are the benefits of WebP resizing?",
      a: "WebP is a highly optimized modern web format. Resizing it retains its high compression ratios while decreasing resolution to make pages load faster."
    },
    {
      q: "Can I lock aspect ratios for WebP?",
      a: "Yes, checking the aspect ratio lock automatically syncs width and height adjustments proportionally."
    },
    {
      q: "Can I export WebP to other formats?",
      a: "Yes, you can select PNG or JPG in the 'Save Image As' dropdown to convert format while resizing."
    },
    {
      q: "Are my WebP files uploaded?",
      a: "No, all file parsing and rendering is done completely client-side in the browser."
    }
  ];
  ...
  useEffect(() => {
    ...
    img.onerror = () => {
      setErrorMsg('Failed to load WebP preview. File may be unsupported or corrupted.');
    };
  ...
  const executeResize = () => {
    ...
    img.onload = () => {
      try {
        ...
        const targetMime = getMimeForSaveFormat(saveFormat, 'image/webp');
        ...
        compressCanvasToBlob(canvas, targetMime, targetBytes)
          .then(({ blob, mime }) => {
            ...
            saveHistory('Resize WebP', `${resizedName} (${targetWidth}x${targetHeight})`);
          })
          .catch((err) => {
            console.error(err);
    ...
    img.onerror = () => {
      setErrorMsg('Failed to process WebP source.');
      setIsResizing(false);
    };
  ...
  return (
    <ToolPageShell
      ...
      steps={_STEPS}
      faqs={_FAQS}
      seoText="Resize WebP images online for free. Set exact dimensions and export as WebP, JPEG or PNG. Browser-based, no uploads."
    >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-3" style={{ background: "#fff", border: "1px solid #E4E4EF", borderRadius: 20, padding: "20px", boxShadow: "0 2px 12px rgba(0,0,0,0.04)", height: "fit-content", display: "flex", flexDirection: "column", gap: 14 }}>
              <div className="flex justify-between items-center pb-2 border-b border-bordercolor">
                <h3 style={{ fontSize: 10, fontWeight: 800, color: "#9898B5", textTransform: "uppercase", letterSpacing: "0.08em" }}>Uploaded WebPs</h3>
