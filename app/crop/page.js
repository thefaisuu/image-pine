...
const _FEATURES = [
  { icon: (<svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="1.8" strokeLinecap="round"><path d="M6 2v14a2 2 0 002 2h14M18 22V8a2 2 0 00-2-2H2"/></svg>), title: 'Freeform Crop', desc: 'Drag to select any region to crop.' },
  { icon: (<svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="1.8" strokeLinecap="round"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/></svg>), title: 'Aspect Ratios', desc: 'Crop to 1:1, 16:9, 4:3 and more.' },
  { icon: (<svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="1.8" strokeLinecap="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg>), title: '100% Private', desc: 'Crops happen entirely in your browser.' },
  ...
  { icon: (<svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="1.8" strokeLinecap="round"><path d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>), title: 'Live Preview', desc: 'Preview the crop in real time.' },
...
export default function CropPage() {
      ...
      try {
        ...
        canvas.toBlob(
          (blob) => {
            ...
            // Log tool history
            const croppedName = file.name.replace(/\.[^/.]+$/, '') + `_cropped.${file.name.split('.').pop()}`;
            saveHistory('Image Cropper', `${croppedName} (${Math.round(sourceWidth)}x${Math.round(sourceHeight)})`);
          },
  ...
  const downloadCroppedImage = () => {
    ...
    const newName = `${baseName}_cropped_${getNaturalCoord(cropBox.width, 'width')}x${getNaturalCoord(cropBox.height, 'height')}.${extension}`;
  ...
  return (
    <ToolPageShell
      ...
      steps={_STEPS}
      faqs={_FAQS}
      seoText="Crop images online for free. Trim JPEG, PNG and WebP to any region or aspect ratio. Browser-based, no uploads."
    >
                          ...
                          >
                            ...
                            {['top-left', 'top-right', 'bottom-left', 'bottom-right'].map((corner) => {
                              ...
                              return (
                                <div
                                  key={corner}
                                  className={`absolute w-3 h-3 bg-primary border border-white rounded-full shadow-md ${classes}`}
                                  onMouseDown={(e) => {
                                    e.stopPropagation();
